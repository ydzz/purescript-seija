...
import Seija.FRP (UpdateType(..), effectBehavior, fetchGlobalKeyEvent, fetchTimeEvent, foldBehavior, tagBehavior)
...
mainMenu rootEv = do
  ...
  _ <- text skin.defaultFont [C.tText "Snake Game",C.cColor white,C.rSize $ vec2 400.0 40.0,C.tFontSize 40,C.tPos $ vec3 0.0 180.0 0.0] (Just elBg)
...
gameScene sheet = do
  ...
  evTimer <- fetchTimeEvent elBg (Time 1.0)
  bDir <- foldBehavior MoveLeft evKey updateDir
  eTickDir <- tagBehavior bDir evTimer
  bSnake <- foldBehavior (SnakeList $ L.fromFoldable [{x:11,y:11},{x:12,y:11},{x:13,y:11}]) eTickDir updateList
  effectBehavior bSnake errorShow
  pure elBg

updateDir::MoveDir -> Tuple Int Boolean -> MoveDir
updateDir dir (10 /\ true) = MoveLeft
updateDir dir (13 /\ true) = MoveRight
updateDir dir (32 /\ true) = MoveUp
updateDir dir (28 /\ true) = MoveDown 
updateDir dir _  = dir

updateList::SnakeList -> MoveDir -> SnakeList
updateList (SnakeList lst@(x:xs)) dir = SnakeList $ (updateFirst x dir):(unsafePartial $ fromJust $ L.init lst)
updateList lst dir = lst

updateFirst :: SnakeNode -> MoveDir -> SnakeNode
updateFirst {x,y} MoveLeft = {x: x - 1, y:y } 
updateFirst {x,y} MoveRight = {x:x + 1, y:y } 
updateFirst {x,y} MoveUp = {x:x, y:y + 1 } 
updateFirst {x,y} MoveDown = {x:x, y:y - 1 } 
