...
import Seija.Component (Prop(..))
import Seija.Component as C
import Seija.Element (emptyElement, spriteB, text)
import Seija.FRP (Event, EventType(..), effectEvent, fetchEvent, foldBehavior, foldMapBehavior, holdBehavior, mergeEvent, tagBehavior)
...
button txt prop parent = do
  skin <- unsafeAskUISkin
  let arr = filter (\(Prop _ _ attrName _) -> attrName == "size") prop
  let sizeProp = if ((length arr) == 0) then ([C.rSize $ vec2 80.0 30.0]) else []
  ...
  _ <- text skin.defaultFont  (sizeProp <> [C.tText txt,C.cColor white,C.tFontSize skin.fontSize]) (Just elSpr)
